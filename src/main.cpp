...
static char const * const g_usage_text = "Ekzekuto : Basic process luncher"
		"<br/>Copyleft: GPL-3.0+<br/>"
		"URL: https://quitter.is/tizenhelper"
		"<br/><br/>"
		"Description: Minimal terminal to execute command and display output."
		"<br/><br/>"
		"Usage: Edit command line and press Execute button"
		"<br/><br/>"
		"Notes:"
		"<br/>"
		...
		"<br/>"
		"Unfortunately there is no fixed width fonts available on platform"
		"<br/><br/>"
...
static int printlog(char* message) {
...
static void exit_cb(void *user_data, Evas_Object *obj, void *event_info) {
	elm_exit();
}

static void oneshoteval_cb(void *user_data, Evas_Object *obj,
		void *event_info) {
...
static void end_func(void *data, Ecore_Thread *thread) {
	appdata_s *ad = (appdata_s *) data;
	ad->thread = 0;
	elm_object_text_set(ad->eval, "Execute! (finished)");
...
static void cancel_func(void *user_data, Ecore_Thread *thread) {
	appdata_s *ad = (appdata_s *) user_data;
	ad->thread = 0;
	elm_object_text_set(ad->eval, "Execute! (cancelled)");
	elm_entry_input_panel_enabled_set(ad->eval, EINA_TRUE);
}
...
static void create_base_gui(appdata_s *ad) {
...

	{
		...
		elm_entry_entry_set(edit, g_usage_text);
